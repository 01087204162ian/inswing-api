...
[구조 가이드]
다음 4가지 항목을 모두 포함하여 완전한 답변을 작성하세요:
1) 원인: 현재 스윙에서 관찰되는 경향·문제 (2-3문장)
2) 해결책: 어떤 동작/순서를 의식해야 하는지 (2-3문장)
3) 느낌: 몸에서 어떤 느낌을 찾아야 하는지 (1-2문장)
4) 연습 방법: 구체적인 연습 루틴 또는 드릴 (2-3문장, 횟수/시간 포함)

**중요**: 반드시 4가지 항목을 모두 완성하여 마무리하세요. 답변 중간에 끊기지 않도록 충분한 길이로 작성하세요.
...
완전한 코칭 답변을 한국어로 작성하세요.
답변은 4가지 항목(원인, 해결책, 느낌, 연습 방법)을 모두 포함하여 자연스럽게 마무리하세요.
`.trim();
}
